...
export const config = schema.parse({
  app: {
    name: "notifications",
    port: process.env.NOTIFICATIONS_PORT ?? 3001,
  },
  cache: {
  ...
  database: {
    write: {
      provider: process.env.NOTIFICATIONS_DB_WRITE_ADAPTER ?? "postgres",
    ...
    read: {
      provider: process.env.NOTIFICATIONS_DB_READ_ADAPTER ?? "mongodb",
      host: process.env.POSTGRES_HOST ?? "localhost",
      port: process.env.POSTGRES_PORT ?? 5432,
      user: process.env.POSTGRES_USER ?? "postgres",
      password: process.env.POSTGRES_PASSWORD ?? "postgres",
