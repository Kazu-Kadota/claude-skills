...
export async function bootstrapExpress() {
  ...
  const mongoWriteConnection = new MongoConnection(config.database.write.uri, "reviews");
  const mongoReadConnection = new MongoConnection(config.database.read.uri, "reviews");
  ...
  const writeRepository =
    config.database.write.provider === "postgres"
      ? new PostgresReviewRepositoryWrite(postgresWriteConnection.getClient())
      : new MongoReviewRepositoryWrite(mongoWriteConnection.getClient().collection<ReviewDTO>("reviews"));

  const readRepository =
    config.database.read.provider === "postgres"
      ? new PostgresReviewRepositoryRead(postgresReadConnection.getClient())
      : new MongoReviewRepositoryRead(mongoReadConnection.getClient().collection<ReviewDTO>("reviews"));

  const cache = new RedisReviewCache(redis);
  const eventBus = new KafkaEventBus(producer);
  const telemetry = new OTelTelemetry();

  const submitReviewUseCase = new SubmitReviewUseCase(writeRepository, cache, eventBus, telemetry);
  const getReviewUseCase = new GetReviewUseCase(readRepository, cache, telemetry);
  const approveReviewUseCase = new ApproveReviewUseCase(readRepository, writeRepository, cache, eventBus, telemetry);
  const rejectReviewUseCase = new RejectReviewUseCase(readRepository, writeRepository, cache, eventBus, telemetry);
  const listReviewsByProductUseCase = new ListReviewsByProductUseCase(readRepository, telemetry);
