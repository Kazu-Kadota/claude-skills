  ...
  providers: [
    ...
    // Postgres write connection
    {
      ...
      useFactory: async () => {
        ...
        const conn = new PostgresConnection(url);
        if (config.database.write.provider === "postgres") await conn.connect();
        return conn;
      },
    },
    {
    ...
    // Postgres read connection
    {
      ...
      useFactory: async () => {
        ...
        const conn = new PostgresConnection(url);
        if (config.database.read.provider === "postgres") await conn.connect();
        return conn;
      },
    },
    {
    ...
    // Mongo write connection
    {
      provide: MONGO_WRITE_CONNECTION,
      useFactory: async () => {
        const conn = new MongoConnection(config.database.write.uri, "inventory");
        if (config.database.write.provider === "mongodb") await conn.connect();
        return conn;
      },
    },
    {
    ...
    // Mongo read connection
    {
      provide: MONGO_READ_CONNECTION,
      useFactory: async () => {
        const conn = new MongoConnection(config.database.read.uri, "inventory");
        if (config.database.read.provider === "mongodb") await conn.connect();
        return conn;
      },
    },
    {
