...
export class DeactivateProductUseCase {
  ...
  private async deactivate(product: Product): Promise<void> {
    product.deactivate();
  ...
  async execute(id: string): Promise<void> {
    return this.telemetry.span("inventory.deactivate", async () => {
      ...
      if (cached) {
        const product = Product.reconstitute(cached);
        await this.deactivate(product);
        return;
      }
      ...
      const product = Product.reconstitute(projection);
      await this.deactivate(product);
    });
  }
}
